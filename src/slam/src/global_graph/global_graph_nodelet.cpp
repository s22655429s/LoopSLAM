...
#include <slam/FloorCoeffs.h>

#include <slam/SaveMap.h>
#include <slam/DumpGraph.h>
...
namespace slam
{
  ...
  public:
    ...
    typedef message_filters::sync_policies::ApproximateTime<nav_msgs::Odometry, sensor_msgs::PointCloud2, sensor_msgs::Image> MySyncPolicy;  //定義一個同步策略
    ...
    virtual void onInit()
    {
      ...
      trans_odom2map.setIdentity();    //初始化為單位矩陣
      ...
      odom_sub.reset(new message_filters::Subscriber<nav_msgs::Odometry>(mt_nh, odom_topic, 256));  //確保節點可以從指定話題接收里程計數據，並且在一個支持多線程處理的環境中有效管理這些數據
      ...
      string img_topic = private_nh.param<std::string>("img_topic", "/kitti/camera_color_left/image_raw");
      ...
      sync2.reset(new message_filters::Synchronizer<MySyncPolicy>(MySyncPolicy(256), *odom_sub, *cloud_sub, *img_sub));  //同步器 sync2 的设置
      sync2->registerCallback(boost::bind(&GlobalGraphNodelet::cloud_callback, this, _1, _2, _3));  //注册回调函数
      ...
      // publishers
      markers_pub = mt_nh.advertise<visualization_msgs::MarkerArray>("/global_graph/markers", 16);  //可視化標記
      odom2map_pub = mt_nh.advertise<geometry_msgs::TransformStamped>("/global_graph/odom2pub", 16);  //兩個坐標系之間的相對關係
      map_points_pub = mt_nh.advertise<sensor_msgs::PointCloud2>("/global_graph/map_points", 1);  //發布處理後的、用於構建地圖的點雲數據
      read_until_pub = mt_nh.advertise<std_msgs::Header>("/global_graph/read_until", 32);  //用於指示接收者處理到指定的時間點的數據
      loop_closure_pub = mt_nh.advertise<visualization_msgs::MarkerArray>("/global_graph/loop_closures", 16);


      dump_service_server = mt_nh.advertiseService("/global_graph/dump", &GlobalGraphNodelet::dump_service, this);  //儲存位姿地圖資訊
      save_map_service_server = mt_nh.advertiseService("/global_graph/save_map", &GlobalGraphNodelet::save_map_service, this);  //儲存地圖點雲資訊

      // 定时器，估计时间间隔定时闭环优化
      double graph_update_interval = private_nh.param<double>("graph_update_interval", 3.0);
      double map_cloud_update_interval = private_nh.param<double>("map_cloud_update_interval", 10.0);
      optimization_timer = mt_nh.createWallTimer(ros::WallDuration(graph_update_interval), &GlobalGraphNodelet::optimization_timer_callback, this);  //this指向GlobalGraphNodelet
   ...
   */
    ...
    {
      const ros::Time &stamp = odom_msg->header.stamp;  //提取里程计消息中的时间戳和序列号
      ...
      Eigen::Isometry3d odom = odom2isometry(odom_msg); //base系在odom系下的变换（map=第一帧keyframe的base,odom2map闭环检测修正值，realtime uodate）
                                                        //将里程计消息转换为 Eigen::Isometry3d
      pcl::PointCloud<PointT>::Ptr cloud(new pcl::PointCloud<PointT>());
      pcl::fromROSMsg(*cloud_msg, *cloud);   //將ROS點雲轉成PCL格式
      if (base_frame_id.empty())   //如果基礎幀 ID 未設置，從點雲息中設置它
      {
        base_frame_id = cloud_msg->header.frame_id;
      }

      odoms.insert(std::pair<int, Eigen::Isometry3d>(seq, odom)); //將當前的序列號和位姿存儲在一個字典或映射中，用於追蹤每個消息的位置。
    ...
    keyframe_queue.push_back(keyframe);   //关键帧队列*/
      ...
      if (keyframe_updater->is_first && keyframe_updater->update(odom))  //檢查當前處理的幀是否為接收到的第一個幀
      {                                                                  //通常會根據傳入的當前位姿決定是否需要更新關鍵幀,檢查自上一個關鍵幀以來的位移或旋轉是否超過了一定的閾值
        w_odom = odom;
        w_cloud.clear();
        w_cloud = *cloud;
        w_img = *img_msg;
      ...
      else if (!keyframe_updater->is_first && keyframe_updater->update(odom))
      {
        ...
        voxelgrid.setLeafSize(0.1f, 0.1f, 0.1f);  //定體素網格的葉子大小為 (0.1, 0.1, 0.1) 米
        pcl::PointCloud<PointT>::Ptr downsampled(new pcl::PointCloud<PointT>());  //創建一個新的 pcl::PointCloud<PointT> 智能指針，用於存儲下採樣後的點雲。
        voxelgrid.setInputCloud(w_cloud.makeShared());  //將工作點雲（w_cloud）設定為 voxelgrid 的輸入
        voxelgrid.filter(*downsampled);  //執行過濾操作
        cv_bridge::CvImageConstPtr cv_ptr = cv_bridge::toCvCopy(w_img, sensor_msgs::image_encodings::MONO8);  // 將 ROS 圖像消息 w_img 轉換為 OpenCV 圖像格式,MONO8 指定圖像為單通道灰度格式
        cv::Mat img = cv_ptr->image.clone();  //cv_ptr 中提取 OpenCV 圖像並進行克隆，以獨立存儲這個圖像
        std::cout << "seq added to keyframs:" << w_seq << std::endl;
        Ptr<cv::Feature2D> detector = cv::ORB::create();  //創建一個 OpenCV 的 ORB 特徵檢測器對象
        std::vector<KeyPoint> keypoints;
        cv::Mat descriptor;
        detector->detectAndCompute(img, Mat(), keypoints, descriptor);  // detectAndCompute 檢測關鍵點並計算它們的描述符keypoint 用於存儲檢測到的關鍵點  descriptor 存儲每個關鍵點對應的描述符
        std::cout << w_seq << " " << w_cloud.size() << " " << downsampled->size() << "-----------------------------------------------------------------------" << std::endl;
        KeyFrame::Ptr keyframe(new KeyFrame(w_img.header.stamp, w_seq, w_odom, accum_d, downsampled, descriptor));  //創建一個新的KeyFrame實例  間戳、序列號、位姿、累計距離、下採樣點雲和描述符
        std::lock_guard<std::mutex> lock(keyframe_queue_mutex);  
        keyframe_queue.push_back(keyframe);  //使用互斥鎖來確保對關鍵幀隊列的操作是線程安全的。將新的關鍵幀添加到隊列中
        w_odom = odom;
        w_cloud.clear();
        w_cloud = *cloud;
        w_img = *img_msg;
      ...
      else if (!keyframe_updater->is_first && !keyframe_updater->update(odom))  //則將當前幀的點雲變換後合併到工作點雲中
      {
        std::lock_guard<std::mutex> lock(keyframe_queue_mutex);
   ...
   */
    ...
    {
      std::lock_guard<std::mutex> lock(keyframe_queue_mutex);  //確保多線程環境下對關鍵幀隊列的訪問是安全的
      ...
      Eigen::Isometry3d odom2map(trans_odom2map.cast<double>());  //從里程計坐標系到地圖坐標系的轉換矩陣
      ...
      for (int i = 0; i < std::min<int>(keyframe_queue.size(), max_keyframes_per_update); i++)
      {
        ...
        new_keyframes.push_back(keyframe);  //從隊列中讀取當前的關鍵幀，並將其添加到 new_keyframes 列表中以進行後續的回環檢測

        // add pose node
        Eigen::Isometry3d odom = odom2map * keyframe->odom;  //計算關鍵幀在地圖坐標系下的位姿
        keyframe->node = graph_slam->add_se3_node(odom); //在 SLAM 圖中添加一個新的節點
        keyframe_hash[keyframe->stamp] = keyframe;  //存儲圖中對應該關鍵幀的節點引用或指針
        ...
        if (keyframes.empty() && new_keyframes.size() == 1)
        {
          ...
          {
            anchor_node = graph_slam->add_se3_node(Eigen::Isometry3d::Identity());  //在graph_slam中添加一個節點,初始姿態是單位矩陣,表示這個節點在原點且無旋轉
            anchor_node->setFixed(true);  //創建的節點設為固定，意味著在後續的優化過程中，這個節點的位置和姿態不會被修改
            anchor_edge = graph_slam->add_se3_edge(anchor_node, keyframe->node, Eigen::Isometry3d::Identity(), Eigen::MatrixXd::Identity(6, 6));
          }  //在graph_slam 中添加一條從 anchor_node 到當前關鍵幀的節點 keyframe->node 的邊
        }
        ...
        Eigen::Isometry3d relative_pose = keyframe->odom.inverse() * prev_keyframe->odom;  //當前關鍵幀 (keyframe) 與前一關鍵幀 (prev_keyframe) 之間的相對姿態。
        Eigen::MatrixXd information = inf_calclator->calc_information_matrix(prev_keyframe->cloud, keyframe->cloud, relative_pose);  //使用 inf_calclator 對象的 calc_information_matrix 方法來根據前後兩個關鍵幀的點雲數據和它們之間的相對姿態計算資訊矩陣
      ...
      std_msgs::Header read_until;
      read_until.stamp = keyframe_queue[num_processed]->stamp + ros::Duration(10, 0);  //keyframe_queue 中处理过的最后一个关键帧 的时间戳加上10秒
      read_until.frame_id = points_topic;
      read_until_pub.publish(read_until);
      read_until.frame_id = "/filtered_points";
      read_until_pub.publish(read_until);

      keyframe_queue.erase(keyframe_queue.begin(), keyframe_queue.begin() + num_processed + 1);  //從 keyframe_queue 中移除已经处理的关键帧
      return true;
    }
   ...
   */
    void floor_coeffs_callback(const slam::FloorCoeffsConstPtr &floor_coeffs_msg)
    {
   ...
   */
    bool flush_floor_queue()
    {
      ...
      auto remove_loc = std::upper_bound(floor_coeffs_queue.begin(), floor_coeffs_queue.end(), latest_keyframe_stamp,
                                         [=](const ros::Time &stamp, const slam::FloorCoeffsConstPtr &coeffs)
                                         {
   ...
   */
    void optimization_timer_callback(const ros::WallTimerEvent &event)
    {
      ...
      visualization_msgs::MarkerArray loop_closure_markers;
      for (const auto &loop : loops)
      {
        Eigen::Isometry3d relpose(loop->relative_pose.cast<double>());  //將 loop->relative_pose（原為 Eigen::Matrix4f）轉換為 Eigen::Isometry3d 類型
        Eigen::MatrixXd information_matrix = inf_calclator->calc_information_matrix(loop->key1->cloud, loop->key2->cloud, relpose);  //計算兩關鍵幀之間的訊息矩陣
        auto edge = graph_slam->add_se3_edge(loop->key1->node, loop->key2->node, relpose, information_matrix);  //添加一個邊 連接兩關鍵針對應的節點 邊的屬性包括相對位姿和訊息矩陣
        ...
        marker.id = loop->key1->node->id();
        marker.type = visualization_msgs::Marker::SPHERE;
        marker.action = visualization_msgs::Marker::ADD;
        marker.pose.position.x = loop->key1->node->estimate().translation().x();
        marker.pose.position.y = loop->key1->node->estimate().translation().y();
        marker.pose.position.z = loop->key1->node->estimate().translation().z();
        marker.pose.orientation.x = 0.0;  // Ensure the quaternion is properly initialized
        marker.pose.orientation.y = 0.0;
        marker.pose.orientation.z = 0.0;
        marker.pose.orientation.w = 1.0;
        // marker.scale.x = 15;  // kitti
        // marker.scale.y = 15;
        // marker.scale.z = 15;
        marker.scale.x = 0.6;      // tb3 
        marker.scale.y = 0.6;
        marker.scale.z = 0.6;
        marker.color.r = 1.0;
        marker.color.g = 0.0;
        ...
        loop_closure_markers.markers.push_back(marker);

         // Log the marker details
        ROS_INFO("Loop closure marker added: id=%d, position=(%f, %f, %f)", 
                 marker.id, 
     ...
     if (!loop_closure_markers.markers.empty()) {
        loop_closure_pub.publish(loop_closure_markers);
        ROS_INFO("Published %lu loop closure markers", loop_closure_markers.markers.size());
    } else {
        ROS_WARN("No loop closure markers to publish.");
    }

      //local 关键帧加入到global关键帧中
      std::copy(new_keyframes.begin(), new_keyframes.end(), std::back_inserter(keyframes));  //使用std::copy 函数来複製 new_keyframes 容器中的所有元素到 keyframes 容器的末尾。
      new_keyframes.clear();

      // optimize the pose graph
      int num_iterations = private_nh.param<int>("g2o_solver_num_iterations", 1024);
      graph_slam->optimize(num_iterations);  //執行圖優化

      //BA is for all keyframes, 把误差均分给每一个node,为了保证相对关系会造成绝对pose的偏差。以first keyframe为基准，消除绝对偏差。
      auto trans_absolute = (keyframes.front()->node->estimate()).inverse();  //世界到第一關鍵幀轉為第一關鍵幀到世界
      for (auto keyframe : keyframes)
      {
        keyframe->node->setEstimate(trans_absolute * keyframe->node->estimate());  //第一關鍵幀到世界*世界到關鍵幀
      }

      // publish tf,tf_odom2map
      const auto &keyframe = keyframes.back();                                          //T map-base * T base-odom
   ...
   * @return
   */
    bool dump_service(slam::DumpGraphRequest &req, slam::DumpGraphResponse &res)
    {
      std::lock_guard<std::mutex> lock(main_thread_mutex);
   ...
   * @return
   */
    bool save_map_service(slam::SaveMapRequest &req, slam::SaveMapResponse &res)
    {
      std::vector<KeyFrameSnapshot::Ptr> snapshot;
  ...
  private:
    ...
    std::deque<slam::FloorCoeffsConstPtr> floor_coeffs_queue;
...
PLUGINLIB_EXPORT_CLASS(slam::GlobalGraphNodelet, nodelet::Nodelet)
